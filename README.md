...
> **Status:** This is a living document. It reflects what I know as of Version 0.4 — momentum and value factors both fully tested, along with a combined version of the two, plus two robustness checks (portfolio concentration, and a two-period split). The period-split check in particular changed how I'd state the overall conclusion — see below. Since then, I've also made the entire project reproducible end-to-end (see "Reproducing this project end-to-end" below): every number on every dashboard page is now generated by code, not typed in by hand. Statistical significance testing and transaction costs are next.

---

**TL;DR:** Does a systematic momentum/value strategy beat a passive benchmark on a 17-stock universe, 2021-2024? On a simple 4-year average, no — the benchmark wins on a risk-adjusted basis against all three active strategies tested. But splitting the same window into two regimes (2021-2022's rate-hike cycle, 2023-2024's AI rally) shows Value and Combined each independently beat the benchmark in their own period — a finding the full-period average hides. Built end-to-end from primary sources: prices via `yfinance`, fundamentals hand-extracted from SEC EDGAR's XBRL filings with point-in-time correctness (no look-ahead bias), backtested with SQL/Python, and fully reproducible — every number in this README and on the dashboard is generated by code, not typed in by hand.
...
I also want to flag directly: two 24-month periods is a thin evidence base statistically. With an effective sample size of two, I can't put a real confidence interval around either sub-period's result, and a genuinely random split could plausibly produce "one win, one loss" by chance alone. I'd treat this as a real, interesting pattern worth investigating further — not yet a validated, statistically confirmed result. Closing that gap with proper significance testing is the next item on this project (see "What's next").
...
- **No trading costs.** Rebalancing every month in real life isn't free. I haven't modeled that yet on any of the three strategies, so this comparison is a bit optimistic across the board. This is next on the list, not an oversight.
...
- **No statistical significance testing yet.** Every comparison in this README is a point estimate, not a confidence interval — see the note at the end of the period-split section above.
...
Every number on every page of the dashboard — Sharpe ratios, holdings, per-stock contributions, rolling metrics — is generated by this pipeline, not typed in by hand. Clone the repo, install dependencies (`pip install -r requirements.txt`), and run these six scripts in order:
...
Each script prints what it computed as it runs. The test suite (`python -m pytest tests/ -v`) covers the core factor-construction and risk-metric logic independently of any live data pull, so it can be run without a network connection or a populated database.
...
│ └── risk.py # Sharpe ratio, volatility, drawdown, rolling Sharpe/volatility
├── tests/
...
- **Statistical significance testing.** Every comparison in this README so far is a point estimate — one Sharpe number against another — with no confidence interval around it. On a 48-month sample (24 per sub-period), a gap like Value's 0.75 versus the benchmark's 0.81 may not be statistically distinguishable from noise. Adding a bootstrap or analytic confidence band around these differences is next, before reading any further into which strategy "really" wins.
- **Transaction costs.** Every result above ignores the cost of actually rebalancing monthly. This is a real, acknowledged limitation, not an oversight — modeling even a simple flat-bps cost assumption is next, alongside significance testing.
